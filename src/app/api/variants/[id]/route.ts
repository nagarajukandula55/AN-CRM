...
// SECURITY: PUT/DELETE below looked a variant up by its Mongo _id ONLY --
...
// permission could edit or delete another business's variant just by
// knowing/guessing its id.
async function assertOwnsVariant(variant: { businessId: unknown } | null, userId: string, isSuperAdmin: boolean, sessionBusinessId: string | null) {
  if (!variant) return false;
  if (isSuperAdmin) return true;
  const authorizedBusinessId = await resolveAuthorizedBusinessId(userId, String(variant.businessId), isSuperAdmin, sessionBusinessId);
  return authorizedBusinessId === String(variant.businessId);
}
...
export async function PUT(
  ...
  try {
    ...
    const existingVariant = await Variant.findById(id).lean();
    if (!existingVariant || !(await assertOwnsVariant(existingVariant as any, session.user.id, !!session.isSuperAdmin, session.business?.businessId || null))) {
