...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.telegramMessageRouting && typeof body.telegramMessageRouting === "object") {
    ...
    for (const key of VENDOR_TELEGRAM_MESSAGE_TYPE_KEYS) {
      ...
      if (entry) {
        routing[key] = { group: !!entry.group, personal: !!entry.personal };
      }
    }
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const type: string = String(body.type || "GENERAL_ANNOUNCEMENT");

  if (!text) {
    return NextResponse.json({ success: false, message: "Message text is required" }, { status: 400 });
  }
  if (!VENDOR_TELEGRAM_MESSAGE_TYPE_KEYS.includes(type)) {
    return NextResponse.json({ success: false, message: "Unknown message type" }, { status: 400 });
  }
