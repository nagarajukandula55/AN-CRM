...
export default function FaultCodesPage() {
  ...
  const { data: faultCodesRes, isLoading: loading, mutate: load } = useSWR(
    `/api/fault-codes${businessId ? `?businessId=${businessId}` : ''}`
  )
  const items: FaultCode[] = faultCodesRes?.success ? faultCodesRes.faultCodes ?? [] : []
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6 bg-bg min-h-screen">
      <PageHeader
        title="Fault Codes"
        description="Master list of device fault / VOC (voice-of-customer) descriptions — populates the Fault Code dropdown when logging a CRM appointment or Workorder, so every technician picks from the same standardized list instead of free-typing it each time."
      />
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {loading ? (
              ...
              <tr><td colSpan={6}><EmptyState kind="empty" title="No fault codes" /></td></tr>
            ) : (
              items.map((f) => (
