...
export default function NewSalesInvoicePage({
  ...
  // A vendor's Owner/Manager can't read the shared platform Business
  // record directly (api/businesses/[id] correctly 403s them -- see its
  // own comment) -- /api/vendor/settings is their own UPI/bank/signature
  // equivalent, scoped to just their VendorProfile. Response shapes differ
  // (flat fields here vs `{business: {...}}` there), normalized into `biz`
  // below so the rest of this page doesn't need to care which it's on.
  const settingsEndpoint = useVendorScope ? '/api/vendor/settings' : (businessId ? `/api/businesses/${businessId}` : null)
  const { data: settingsRaw, mutate: refetchBusiness } = useSWR(settingsEndpoint)
  const biz = useVendorScope ? settingsRaw : settingsRaw?.business
  ...
  return (
    <div className="min-h-screen bg-bg text-ink p-6">
      <PageHeader
        ...
        actions={
          <>
            <Button variant="secondary" size="sm" onClick={() => router.push(listPath)} icon={<ArrowLeft className="w-4 h-4" />}>Cancel</Button>
