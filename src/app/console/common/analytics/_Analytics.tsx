...
export default function AnalyticsPage({
  salesPath = '/console/common/sales',
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <div className="grid-cards-auto">
            <StatCard label="Total Revenue" value={fmt(data.revenue.total)} sub={`${data.revenue.totalInvoices} paid invoices`} href={salesPath} />
            <StatCard label={`This Month (${new Date().toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })})`} value={fmt(data.revenue.thisMonth)} sub={`${data.revenue.thisMonthInvoices} invoices`} href={salesPath} />
            <StatCard label="Invoices" value={String(data.revenue.totalInvoicesAllStatuses)} sub="all statuses" href={salesPath} />
            <StatCard label="Total Workorders" value={String(data.operations.totalWorkorders)} href={jobsheetsPath} />
            <StatCard label="Open Workorders" value={String(data.operations.openWorkorders)} href={`${jobsheetsPath}?quick=OPEN`} />
            <StatCard label="Closed Workorders" value={String(data.operations.closedWorkorders)} href={`${jobsheetsPath}?status=CLOSED`} />
          </div>
