...
// Raw shape /api/inventory/items actually returns (models/Inventory.js) --
// distinct from the UI-facing InventoryItem below. Field names never
// matched (itemName vs name, onHandQuantity vs quantity, no category/
// basePrice at all), so this page never rendered real data even before --
// normalized in the mapping below instead of changing every render site.
...
function normalizeItem(raw: RawInventoryDoc): InventoryItem {
  return {
    _id: raw._id,
    name: raw.itemName || 'Unnamed item',
    sku: raw.sku,
    category: undefined,
    quantity: raw.onHandQuantity ?? 0,
    unit: raw.unit,
    reorderLevel: raw.reorderLevel,
    basePrice: raw.averageCost,
...
export default function InventoryPage({ backPath = '/console', stockAdjustmentsPath }: { backPath?: string; stockAdjustmentsPath?: string }) {
  ...
  // raw docs normalized to this page's own field names (see normalizeItem).
  const rawItems: RawInventoryDoc[] = itemsRes
    ? Array.isArray(itemsRes)
      ? itemsRes
      : itemsRes.data ?? itemsRes.items ?? []
    : []
  const items: InventoryItem[] = rawItems.map(normalizeItem)
    ...
    <div className="min-h-screen bg-bg text-ink">
      <div className="px-6 py-10">
        <PageHeader
          ...
          actions={<Button variant="secondary" size="sm" onClick={() => router.push(backPath)} icon={<ArrowLeft className="w-4 h-4" />}>Back</Button>}
        />
        ...
        {/* Quick Links */}
        {stockAdjustmentsPath && (
          <div className="flex gap-3 mb-6">
            <Link href={stockAdjustmentsPath}>
