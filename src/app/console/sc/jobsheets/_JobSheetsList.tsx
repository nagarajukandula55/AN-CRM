...
// basePath lets this same component render inside both /console/sc/jobsheets
// and /vendor/crm/jobsheets -- same pattern as the new/[id] form and the
// dashboard. /api/crm/jobsheets is already vendor-scoped.
export default function JobSheetsListPage({ basePath = '/console/sc/jobsheets' }: { basePath?: string } = {}) {
  return (
    <Suspense fallback={null}>
      <JobSheetsListPageInner basePath={basePath} />
    </Suspense>
  )
}

function JobSheetsListPageInner({ basePath }: { basePath: string }) {
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            ...
            <Button onClick={() => router.push(`${basePath}/new`)} icon={<Plus className="w-4 h-4" />}>
              Create Workorder
            </Button>
          </div>
        }
