...
// Brand's own New Job Sheet modal -- deliberately a SEPARATE component from
// SCNewJobSheetModal, not one shared modal branching on operatingMode.
// Brand is a call-center queue across multiple centers: it keeps every
// dropdown (Appointment Type, Request Type, Service Center/Warehouse
// routing, Fault/Symptom Code taxonomy) and the full shared-catalog picker
// whose "Request to add" flow goes through Super Admin approval. SC's
// modal drops all of that -- see SCNewJobSheetModal's own top comment.
//
// Larger than the app's usual form scale on purpose -- this modal replaces
// a full page (see jobsheets/page.tsx routing to here instead of /new),
// and a dense multi-section intake form is exactly the case where bigger
// text/inputs actually matter for legibility, not just consistency.
...
export function BrandNewJobSheetModal({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
      <div className="relative w-full max-w-4xl max-h-[92vh] bg-surface border border-border rounded-card flex flex-col overflow-hidden shadow-card-lg">
        <div className="flex items-center justify-between px-8 py-6 border-b border-border">
          <div>
            <h2 className="h-section text-xl">New Job Sheet</h2>
            <p className="text-sm text-ink-3 mt-1">For a direct walk-in — no appointment needed first.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
          ...
          <section className="rounded-card border border-border bg-surface-2/40 p-6 space-y-4">
