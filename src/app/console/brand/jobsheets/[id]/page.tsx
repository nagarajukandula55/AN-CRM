...
export default function JobSheetDetailPage() {
  ...
  const { data: jobRes, isLoading: loading, error: jobSwrError, mutate: fetchJob } = useSWR(
    id ? `/api/crm/jobsheets/${id}` : null
  )
  const job: JobSheet | null = jobRes && jobRes.success !== false ? jobRes.jobSheet : null
  const error: string | null = jobSwrError
    ? (jobSwrError.message || 'Could not load workorder.')
    : (jobRes?.success === false ? (jobRes.message || 'Failed to load workorder') : null)
  ...
  // Line items come back with faultCodeId/symptomCodeId/solutionId populated
  // (object with code/description, for the print page) -- normalize each
  // back to a plain id string here since the editable dropdowns below
  // compare against id strings, not populated objects. Re-syncs from the
  // server every time the SWR data changes (e.g. after a save/action calls
  // fetchJob()), same as the previous manual fetchJob did.
  useEffect(() => {
    if (!job) return
    const idOf = (v: any) => (v && typeof v === 'object' ? v._id : v) || undefined
    const normalizedLines = (job.lineItems || []).map((l: any) => ({
      ...l,
    ...
    setLineItems(normalizedLines.length ? normalizedLines : [{
      ...
      faultCodeId: idOf((job as any).faultCodeId),
    }])
    setServiceCharge((job as any).serviceCharge || 0)
    setRemark(job.remark || '')
    setBrandJobNo(job.brandJobNoForPartOrder || '')
  }, [job])

  const { data: solutionsData } = useSWR(job?.businessId ? `/api/solutions?businessId=${job.businessId}` : null)
  useEffect(() => { if (solutionsData) setSolutions(solutionsData.solutions || []) }, [solutionsData])

  const { data: symptomCodesData } = useSWR(job?.businessId ? `/api/symptom-codes?businessId=${job.businessId}` : null)
  useEffect(() => { if (symptomCodesData) setSymptomCodes(symptomCodesData.symptomCodes || []) }, [symptomCodesData])

  const { data: faultCodesData } = useSWR(job?.businessId ? `/api/fault-codes?businessId=${job.businessId}` : null)
  useEffect(() => { if (faultCodesData) setFaultCodes(faultCodesData.faultCodes || []) }, [faultCodesData])

  // Fixed Service Charge editing is restricted to Owner/Manager, per
  // explicit direction -- everyone else sees it read-only.
  // (canEditServiceCharge is derived below, once /api/auth/me has loaded.)
  ...
  const { data: businessData } = useSWR(job?.businessId ? `/api/businesses/${job.businessId}` : null)
  const inventorySerialized = Boolean(businessData?.business?.inventorySerialized)
  ...
  const meRole = String(meUser?.role || '').toUpperCase()
  const canEditServiceCharge = Boolean(meUser?.isSuperAdmin) || meRole.includes('OWNER') || meRole.includes('MANAGER')
  const currentUserId: string | null = meUser?.id || null
  const isSuperAdminUser = Boolean(meUser?.isSuperAdmin)
  ...
  const { data: engineersData } = useSWR(id ? `/api/crm/jobsheets/${id}/engineers` : null)
  const engineers: StaffMember[] = engineersData?.success ? (engineersData.engineers || []) : []
  ...
  const bomBrandId = job ? (typeof job.brandId === 'object' ? job.brandId?._id : job.brandId) : undefined
  const bomDeviceModelId = job ? (typeof job.deviceModelId === 'object' ? job.deviceModelId?._id : job.deviceModelId) : undefined
  const bomParamsStr = (() => {
    const params = new URLSearchParams()
    if (bomBrandId) params.set('brandId', bomBrandId)
    if (bomDeviceModelId) params.set('deviceModelId', bomDeviceModelId)
    return params.toString()
  })()
  const { data: bomPartsData } = useSWR(job ? `/api/service-center-bom${bomParamsStr ? `?${bomParamsStr}` : ''}` : null)
  ...
  if (error || !job) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-3">
        ...
        <button onClick={() => router.push('/console/brand/jobsheets')} className="text-sm text-gray-500 underline">Back to Workorders</button>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <div className="px-6 py-10">
        <div className="flex items-center gap-4 mb-6">
          <button onClick={() => router.push('/console/brand/jobsheets')} className="w-9 h-9 rounded-xl border border-gray-200 bg-white flex items-center justify-center hover:bg-gray-100 transition">
          ...
          <div className="ml-auto flex gap-2">
            ...
            {job.invoiceId && (
              <button
                onClick={() => router.push(`/console/common/documents/crm-invoices/${job.invoiceId}`)}
