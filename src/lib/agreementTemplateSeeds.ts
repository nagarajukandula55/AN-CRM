// Standard-form agreement template content -- see
// scripts/migrateAgreementTemplatesToCentral.ts (one-time push of this
// content into central-api's shared "agreementtemplates" dataset) and
// api/agreements/templates/route.ts (the consumer). Split into its own
// plain lib file, NOT the route file, because Next.js App Router route
// files only allow specific exports (GET/POST/etc.) -- an arbitrary
// named export here broke the production build.
//
// This is a standard-form draft, not reviewed by a qualified advocate
// for any specific use -- get real legal review before using it for a
// binding engagement.
